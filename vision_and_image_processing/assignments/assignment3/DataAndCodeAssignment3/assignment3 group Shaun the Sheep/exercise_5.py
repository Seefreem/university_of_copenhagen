...
I, mask, S = read_data_file('shiny_vase2.mat')
...
for row in range(m):
    for col in range(n):
        if mask[row, col]:
            ...
            data = (I[row, col, :], S)
            modulated_normal, inliers, best_fit = ransac_3dvector(data,2.0,
...
#use smooth_normal_field(iters=6) and plot normal image and depth image
(n1, n2, n3) = smooth_normal_field(normals_matrix[:,:,0], normals_matrix[:,:,1], normals_matrix[:,:,2], mask, iters=6)
...
#use smooth_normal_field(iters=150) and plot normal image and depth image
